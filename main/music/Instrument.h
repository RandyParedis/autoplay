...
/**
 * The instruments namespace contains all existing music instruments in MIDI as
 * Instrument objects.
 *
...
static std::shared_ptr<Instrument> acoustic_grand_piano = std::make_shared<Instrument>("Acoustic Grand Piano", 1, 1, 0);
static std::shared_ptr<Instrument> bright_acoustic_piano =
    std::make_shared<Instrument>("Bright Acoustic Piano", 1, 2, 0);
static std::shared_ptr<Instrument> electric_grand_piano = std::make_shared<Instrument>("Electric Grand Piano", 1, 3, 0);
static std::shared_ptr<Instrument> honky_tonk_piano     = std::make_shared<Instrument>("Honky-tonk Piano", 1, 4, 0);
static std::shared_ptr<Instrument> electric_piano1      = std::make_shared<Instrument>("Electric Piano 1", 1, 5, 0);
static std::shared_ptr<Instrument> electric_piano2      = std::make_shared<Instrument>("Electric Piano 2", 1, 6, 0);
static std::shared_ptr<Instrument> hapsichord           = std::make_shared<Instrument>("Hapsichord", 1, 7, 0);
static std::shared_ptr<Instrument> clavinet             = std::make_shared<Instrument>("Clavinet", 1, 8, 0);

/**
 * The all vector is a list of all the above instruments, indexable by 0, or by
 * name,
 ...
 * @return nullptr if no Instrument was found, otherwise a pointer to the
 * Instrument
 */
